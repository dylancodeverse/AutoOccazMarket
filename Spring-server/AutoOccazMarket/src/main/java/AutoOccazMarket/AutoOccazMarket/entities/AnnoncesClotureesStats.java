...
public class AnnoncesClotureesStats {
    ...
    public static AnnoncesClotureesStats[] sortBy(Connection connection , String by ,String order) throws Exception {
        Statement statement = connection.createStatement();
        ResultSet re = statement.executeQuery(
                "WITH V_modeles_annonces AS (SELECT modeles.id_modeles, modeles.nom_modele, annonces.id_annonce, annonces.etat_general, annonces.etat_validation, annonces.localisation, annonces.modeles_id_modeles, annonces.utilisateur_id_utilisateur, annonces.prix FROM modeles JOIN annonces ON modeles.id_modeles = annonces.modeles_id_modeles WHERE annonces.etat_validation = 30), V_annonces_commission AS (SELECT V_modeles_annonces.*, commission.pourcentages FROM V_modeles_annonces JOIN commission ON V_modeles_annonces.id_annonce = commission.annonces_id_annonce) , v_end as (SELECT nom_modele, COUNT(*) AS nbreVoitureVendu, SUM(prix - (prix * pourcentages/100)) AS prixvendu FROM V_annonces_commission GROUP BY nom_modele), v_end2 as (select * from v_end union all  select nom_modele , 0 as nbrevoiturevendu , 0 prixvendu from modeles)select nom_modele , sum(nbrevoiturevendu) as nbrevoiturevendu ,sum(prixvendu) as prixvendu from  v_end2 group by nom_modele ORDER BY "+by+" "+order);
        ArrayList<AnnoncesClotureesStats> l = new ArrayList<>();
